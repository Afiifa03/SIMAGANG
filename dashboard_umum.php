    ...
    <div class="container py-4">
        ...
        <div class="card text-center shadow mb-4">
            <div class="card-body">
                ...
                <?php if (!$is_admin && !$is_peserta): ?>
                    <!-- Jika belum login (bukan admin & bukan peserta) -->
                    <a href="login" class="btn btn-primary">Masuk dan Daftar Sekarang</a>

                <?php elseif ($is_admin): ?>
                    <!-- Jika login sebagai admin -->
                    <span class="badge bg-success mb-2">
                        Login sebagai: <?= htmlspecialchars($_SESSION['user']['nama'] ?? 'Admin'); ?>
                    </span><br>

                <?php elseif ($is_peserta): ?>
                    <!-- Jika login sebagai peserta -->
                    <!-- Tidak menampilkan tombol apapun -->
                <?php endif; ?>
            </div>
        </div>
                        ...
                        <ul class="list-group">
                            <?php foreach ($universitas as $univ): ?>
                                <li class="list-group-item d-flex align-items-center">
                                    <span
                                        class="badge rounded-circle me-2 <?= $univ['status'] === 'aktif' ? 'bg-success' : 'bg-secondary'; ?>">&nbsp;</span>
                                    <?= $univ['nama']; ?>
                                </li>
                            <?php endforeach; ?>
                        </ul>
